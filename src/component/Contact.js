const Contact = () => {
    return(
        <section className="bg-danger">
        <div className="container-fluid">
        <div className="row"><h1>Contact</h1></div>
        <div className="row">
          <div className="col-12 col-sm-6"><img src="http://trucsetastucesjeux.com/wp-content/uploads/2020/04/Remake-de-Final-Fantasy-7-comment-battre-le-boss-final.jpg" witdh="100%"></img></div>
          <div className="row">
          <div className="col-12 col-md-6">Elit veniam excepteur deserunt aliquip ea sint laboris est ut sint deserunt ex fugiat. Minim ad anim adipisicing cupidatat adipisicing proident aliqua quis ex. Qui dolore tempor ipsum voluptate eu ex aliquip esse in qui.
          </div></div>
        </div>
        </div>
      </section>
    )
}

export default Contact